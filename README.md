...
Разработать REST API сервис карточек выпусков журналов на базе фреймворка **Express.js** со следующими эндпоинтами:
...
**Вариант:** карточки выпусков журналов («ИНТЕРНЫУКА») с полями `id`, `img`, `title`, `deadline`, `shortDeadline`, `releaseDate`, `elibraryDate`, `mailDate`, `trackDate`.
...
Инкапсулирует работу с файловой системой. Предоставляет методы `readData` и `writeData` для работы с файлами.
...
```js
// server/src/services/stocksService.js
...
`server/src/index.js` — точка входа. Здесь подключаются middleware, роутеры и глобальные обработчики ошибок.
...
### GET /stocks?title=Выпуск 14 — фильтрация по названию
...
**Задание:** реализовать проверку наличия всех обязательных полей (`title`, `deadline`, `releaseDate`) при создании карточки. При отсутствии хотя бы одного поля — вернуть ошибку `400 Bad Request`.
...
**Задание:** написать собственный middleware, который логирует каждый входящий запрос — метод, URL и время в формате ISO. Middleware должен передавать управление следующему обработчику.

Реализовано в точке входа `server/src/index.js` с вызовом `next()`:
